...
examples = """examples:
    ...
    ./funclatency do_sys_open -o open.pkl  # time the do_sys_open() kernel function and store the latency distribution in open.pkl
...
parser.add_argument('-o','--output', type=str, default='', help='Write the latency distribution to the file OUTPUT')
...
BPF_HISTOGRAM(ret_value, int, 10000); // 10KB
STORAGE
FUNCTION
...
int trace_func_return(struct pt_regs *ctx)
{
    ...
    // check if the return value matches 
    CHECK 
...
# added by xzhu
if args.ret != -1:
    bpf_text = bpf_text.replace('CHECK',
            """
    if (ret != %d) {return 0; }
            """ % args.ret)
else:
...
while (1):
    ...
    return_vals = [0] * 10000 # bytes
    for k, v in ret.items():
        if k.value >= 10000:
            continue
        if v.value > 0:
            print(v.value, k.value)
    ...
    # If the return value cannot be used. Get the N most time-consuming function invocations
    latency_vals = []
    if args.number:
        num_calls = args.number
        count = 0
        for i, d in reversed(list(enumerate(latency_hist))):
            if count + d > num_calls:
        ...
        for i, d in enumerate(latency_hist):
            if d != 0 and i > args.lowerbound*1000:
                for _ in range(0, d):
                    latency_vals.append(i)
    ...
    # Dump the latency distribution 
    if args.output:
        with open(args.output, "wb") as fout:
           pickle.dump(latency_vals, fout)
